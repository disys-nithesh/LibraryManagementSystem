...
<body>
	...
	<h1>DELETE BOOK:</h1>
	<div class="container">
		<form class="form-inline" action="delete.jsp" method="get"
	...
	<form class="form-inline" action="delete.jsp" method="get"
		...
		<input type="text" name="id" class="form-control"
			...
			value="<%=result.getString("author")%>" readonly> <input
			type="text" name="year" class="form-control"
			value="<%=result.getInt("released_year")%>" readonly>
		<button type="submit" value="Update" name="save">DELETE</button>
	</form>
	<script>
		function myFunction() {
	...
	String query1 = "DELETE FROM book_details WHERE book_id="+id;
	try {
	Class.forName("com.mysql.cj.jdbc.Driver");
	} catch (ClassNotFoundException e) {
	e.printStackTrace();
